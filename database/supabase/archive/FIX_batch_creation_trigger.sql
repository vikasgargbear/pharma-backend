...
-- First, restore the trigger if it was disabled
CREATE TRIGGER IF NOT EXISTS trigger_create_batches_from_purchase
...
BEGIN
    ...
    LOOP
        ...
        IF v_item.batch_number IS NULL OR v_item.batch_number = '' THEN
            -- Auto-generate batch number: SUP-YYYYMMDD-PRODID-RANDOM
            v_batch_number := 'AUTO-' || 
...
-- Also update the API's batch creation to avoid duplicates
COMMENT ON FUNCTION create_batches_from_purchase() IS 
'Enhanced: Auto-generates batch numbers when null and avoids duplicates';
